...
public class AnyXPCOM {
	 ...
	 */
	public static void initBrowser(final JavaFXBrowser browser) {
		browser.execute(
			"if (!window.nsiArray) {" +
				"window.nsiArray = [];" +
	...
	public static <T> T queryInterface(String jsExpression, Class<T> type, JavaFXBrowser browser) {
		String nsiObject = "return convertNsi(" + jsExpression + ")";
		long delta = System.nanoTime();
		Object evaluateResult = browser.evaluate(nsiObject);
		delta = System.nanoTime() - delta;
		nanoSum2+=delta;
		counter2++;
		return convertFromNsi(evaluateResult, type, browser);
	}
	
	@SuppressWarnings("unchecked")
	private static <T> T createProxy(JavaFXBrowser browser, int id, Class<T> type) {
